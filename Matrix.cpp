...
Matrix operator*(const Matrix &x, const Matrix &y)
{
    ...
    Matrix res(x.nRows(), y.nCols(), 1);
    for (int row = 0; row < x.nRows(); row++)
    {
        for (int col = 0; col < y.nCols(); col++)
        {
            for (int k = 0; k < x.nCols(); k++)
            {
                res(row,col) += (x.get(row, k) * y.get(k, col));
            }
        }
    }
    return res;
...
Matrix cholsekyDecomposition(const Matrix &m)
{
    ASSERT(m.nRows() == m.nCols());
    Matrix l(m.nRows(), m.nCols(), 1);
    for (int row = 0; row < l.nRows(); row++)
    {
        for (int col = 0; col <= row; col++)
        {
            ...
            for (int k = 0; k < col; k++)
            {
                sum += (l.get(col, k)) * (l.get(row, k));
            }
            if (row == col)
            {
                double value = sqrt(m.get(row, col) - sum);
                l.set(row, col, value);
            }
            else
            {
                double value = (m.get(row, col) - sum) / l.get(col, col);
                l.set(row, col, value);
            }
        }
    }
...
{
    Matrix m1("1,1;1,1;1,1");
    Matrix m2("2,2;2,2");
    Matrix actual = m1 * m2;
    Matrix expected("4,4;4,4;4,4");
    expected.assertEquals(actual, 0.001);
}

static void testCholsekyDecomposition()
{
    Matrix m("4,12,-16;12,37,-43;-16,-43,98");
    Matrix actual = cholsekyDecomposition(m);
    Matrix expected("2,0,0;6,1,0;-8,5,3");
    expected.assertEquals(actual, 0.001);
}
...
void testMatrix()
{
    ...
    TEST(testCholsekyDecomposition);
}
